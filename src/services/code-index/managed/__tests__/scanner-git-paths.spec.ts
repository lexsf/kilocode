...
describe("Scanner Git Path Handling", () => {
	...
	it("should convert relative git paths to absolute paths for feature branches", async () => {
		...
		vi.mocked(gitUtils.getCurrentBranch).mockReturnValue("feature/test")
		vi.mocked(gitUtils.getGitDiff).mockReturnValue({
		...
		const diff = gitUtils.getGitDiff("feature/test", "main", workspacePath)
